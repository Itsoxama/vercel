...
export async function GetLocations() {
  const apiUrl = "https://cfsapi.infibrain.com/api/Company/GetLocations";
  const res = await fetch(apiUrl, { cache: 'no-store' });
  const data = await res.json();
  return data;
}
...
// lib/api.js
export async function getJobsByFilters({workMode = [""],token="", experienceLevel = "", locationsearch = "", sector = [""], industry = [""], jobType = [""], search = "", page = 1, limit = 10 }) {
...
export async function savejob(token="",id) {
  const apiUrl = `/api/cfs/api/Company/SaveJob?jobId=${id}`;
    const headers = {
    "Content-Type": "application/json",
    "accept": "*/*",
  };
